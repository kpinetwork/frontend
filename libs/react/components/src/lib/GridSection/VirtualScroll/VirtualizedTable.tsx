...
function MuiVirtualizedTable({
  ...
  const areObjectsColumnsEqual = !equals(columnsState, parseStringifyToJson({ defaultValue: columns }));

  useEffect(() => {
    if (userName && areObjectsColumnsEqual) {
  ...
  }, [userName, areObjectsColumnsEqual]);
  ...
  if (!userName || columnsState.length !== columns.length) {
    return <Loader />;
  }
